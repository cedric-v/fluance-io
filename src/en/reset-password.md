...
<div class="min-h-screen bg-[#fdfaf6] py-12 px-4 sm:px-6 lg:px-8">
  <div class="max-w-md mx-auto bg-white rounded-lg shadow-lg p-8 section-card">
    <div class="text-center mb-8">
      <h1 class="text-3xl font-bold text-[#0f172a] mb-2">Reset Password</h1>
    ...
    <div class="mt-6 text-center space-y-2">
      <p class="text-sm text-[#1f1f1f]/80">
        <a href="/en/firebase-login" class="text-[#82153e] hover:text-[#82153e]/80 font-medium">Back to login</a>
      </p>
    </div>
  </div>
</div>
  ...
  if (actionCode && mode === 'resetPassword') {
    ...
    requestForm.addEventListener('submit', async function(e) {
      ...
      try {
        ...
        if (result.success) {
          successDiv.querySelector('p').textContent = 'A password reset email has been sent to ' + email + '. Check your inbox and click the link to reset your password.';
          successDiv.classList.remove('hidden');
        } else {
          errorDiv.querySelector('p').textContent = result.error || 'Error sending email.';
          errorDiv.classList.remove('hidden');
        }
      } catch (error) {
        console.error('Error:', error);
