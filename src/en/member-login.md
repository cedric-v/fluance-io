...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  // Check if automatic login is requested (after password reset)
  ...
  if (autoLoginEmail && autoLoginPassword && autoLoginTimestamp) {
    // Check that timestamp is not too old (max 5 minutes)
    ...
    if (now - timestamp < 5 * 60 * 1000) {
      ...
      // Pre-fill form and login automatically
      // Use the real HTML form so Safari detects the fluance.io domain
      ...
      if (emailInput && passwordInput && form) {
        ...
        // Wait a short delay for Safari to detect filled fields
        setTimeout(async () => {
          try {
            ...
            // Login via API (HTML form will allow Safari to detect the domain)
            ...
            if (result.success) {
              ...
              showError(result.error || 'Error during automatic login.', result);
            }
          } catch (error) {
            console.error('Error during automatic login:', error);
            showError('An error occurred during automatic login.');
          }
        }, 300);
