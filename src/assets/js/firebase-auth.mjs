...
async function sendSignInLink(email, actionCodeSettings = null) {
  ...
  try {
    ...
    // Essayer d'abord Mailjet (via Cloud Function)
    try {
      console.log('[Firebase Auth] Tentative d\'envoi via Mailjet (Cloud Function)...');
      ...
      const result = await sendSignInLinkViaMailjet({ email });
      
      if (result.data && result.data.success) {
        console.log('[Firebase Auth] ✅ Lien de connexion envoyé via Mailjet');
        if (result.data.isClient === false) {
           console.log('[Firebase Auth] ℹ️ Note: Utilisateur non client, email de redirection envoyé');
        }
        return { success: true, message: 'Lien de connexion envoyé avec succès via Mailjet.' };
      }
      
      // Si la fonction ne renvoie pas success: true sans erreur, on continue vers le fallback
      console.warn('[Firebase Auth] La fonction Mailjet n\'a pas retourné de succès explicite, passage au fallback...');
      
    } catch (mailjetError) {
      console.warn('[Firebase Auth] ⚠️ Erreur lors de l\'appel à la Cloud Function Mailjet:', mailjetError);
      console.warn('[Firebase Auth] Passage au fallback standard Firebase Auth...');
    }

    // Fallback sur Firebase Auth (SDK standard)
    console.log('[Firebase Auth] Appel de auth.sendSignInLinkToEmail (Fallback SDK)...');
...
// Cache session pour éviter les lectures Firestore redondantes
let _cachedUserProducts = null;
let _cachedUserUid = null;
// Cache des documents protectedContent (contentId -> données) pour éviter
// un re-fetch Firestore lors du changement d'onglet : le contenu de chaque
// onglet a déjà été chargé par loadProtectedContent() (sans contentId).
...
async function loadProtectedContent(contentId = null) {
  try {
    ...
    if (_cachedUserProducts && _cachedUserUid === user.uid) {
      userProducts = _cachedUserProducts;
    } else {
      ...
      _cachedUserProducts = userProducts;
    ...
    if (contentId) {
      // Utiliser le cache session si le contenu a déjà été chargé par
      // loadProtectedContent() (sans contentId). Cela évite un re-fetch
      // Firestore à chaque changement d'onglet et rend l'affichage instantané.
      const cached = _cachedContentDocs[contentId];
      if (cached) {
        // Vérifier que l'utilisateur possède toujours le produit associé
        const userProductData = userProducts.find(p => p.name === cached.product);
        if (!userProductData) {
          return {
            success: false,
        ...
        // Vérifier l'accès progressif
        if (!cached.isAccessible) {
          if (cached.product === '21jours' && cached.daysRemaining !== null) {
            return {
              success: false,
              error: `Ce contenu sera disponible dans ${cached.daysRemaining} jour${cached.daysRemaining > 1 ? 's' : ''}. Vous êtes actuellement au jour ${cached.currentDay} du défi de 21 jours.`,
          ...
          if (cached.product === 'complet' && cached.weeksRemaining !== null) {
            return {
              success: false,
              error: `Ce contenu sera disponible dans ${cached.weeksRemaining} semaine${cached.weeksRemaining > 1 ? 's' : ''}. Vous êtes actuellement à la semaine ${cached.currentWeek}.`,
              errorCode: 'CONTENT_NOT_AVAILABLE_YET',
              suggestion: 'Continuez à suivre le programme semaine par semaine. Le contenu se débloque automatiquement chaque semaine.',
              weeksRemaining: cached.weeksRemaining,
...
async function displayProtectedContent(contentId, containerElement) {
  try {
    ...
    // Utiliser le cache si disponible pour éviter une lecture Firestore redondante
    let userProductNames = null;
    const user = auth.currentUser;
    if (user) {
      if (_cachedUserProducts && _cachedUserUid === user.uid) {
        userProductNames = _cachedUserProducts.map(p => p.name);
      } else {
        try {
          ...
          if (userDoc.exists) {
            ...
            userProductNames = userProducts.map(p => p.name);
            _cachedUserProducts = userProducts;
            _cachedUserUid = user.uid;
          }
        } catch (error) {
