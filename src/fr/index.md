...
<!-- Note: Le script MailJet devrait détecter automatiquement les clics sur les éléments avec data-mailjet-trigger -->
...
<script>
  ...
  (function() {
    ...
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', initMailJet);
    } else {
      initMailJet();
    }
    ...
    function initMailJet() {
      ...
      const checkMailJet = setInterval(function() {
        ...
        // Vérifier différentes APIs possibles
        const mailjetAPI = window.mjPopin || window.mailjet || window.mj || (typeof mjPopin !== 'undefined' ? mjPopin : null);
        
        if (mailjetAPI || attempts >= maxAttempts) {
          clearInterval(checkMailJet);
          
          if (mailjetAPI) {
            console.log('API MailJet détectée:', Object.keys(mailjetAPI));
          } else {
            console.log('API MailJet non détectée après', attempts * 100, 'ms');
          }
          ...
          triggerButtons.forEach(button => {
            button.addEventListener('click', function(e) {
              ...
              // Si aucune API n'est disponible, essayer via l'iframe trigger
              const triggerIframe = document.querySelector('iframe[data-w-type="trigger"]');
              if (triggerIframe) {
                // Essayer plusieurs méthodes de déclenchement
                try {
                  // Méthode 1: postMessage
                  if (triggerIframe.contentWindow) {
                ...
                // Méthode 2: Simuler un clic sur l'iframe (si accessible)
                try {
                  if (triggerIframe.contentDocument) {
                    const triggerButton = triggerIframe.contentDocument.querySelector('button, a, [data-trigger]');
                    if (triggerButton) {
                      triggerButton.click();
                    }
                  }
              ...
              console.log('Tentative de déclenchement MailJet. API disponible:', !!mailjetAPI);
              if (mailjetAPI) {
                console.log('Méthodes disponibles:', Object.keys(mailjetAPI));
              }
            });
          });
        }
